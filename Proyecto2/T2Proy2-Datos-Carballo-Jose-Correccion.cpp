// Estudiantes: Jos� Carballo Mart�nez
...
// Define un comando para limpiar/borrar la terminal
    ...
    #define LIMPIAR "cls"
#else // Si se compila en Linux o Mac
...
static void crearReporte(string nombreReporte, string distintivo, string texto)
{
	...
	// Comprueba si el archivo se abr� correctamente
	...
	string llave, string nombre)
{
	...
	else
	{
		if (llave < ra->codProducto)
		{
	        ...
	        if (Hh) // Hubo crecimiento en la altura del sub�rbol izquierdo
				Equilibrar2(ra, Hh);
	    }
	    else if (llave > ra->codProducto)
		{
	        ...
	        if (Hh) // Hubo crecimiento en la altura del sub�rbol derecho
				Equilibrar1(ra, Hh);
	    }
	    else
        	Hh = false; // La altura no cambi�
	}
}
...
pnodoAAInventario ArbolAAInventarios::buscarInventario(string codInventario) const
{
    ...
    return NULL; // No se encontr� el inventario en el �rbol
}
...
bool ArbolBUsuarios::loginUsuario(string cedula)
{
    ...
    for (int i = 0; i < usuario->valores.size(); i++)
	{
	    string dato = usuario->valores[i];
	    int pos = dato.find(";");
    ...
    ArbolAVLProductos& arbolProductos)
{
    ...
    while (getline(archivo, linea))
    {
        ...
        if (arbolPasillos.buscarPasillo(codPasillo) != NULL
            ...
            // Inserta en el �rbol RN de marcas despu�s de las validaciones
    ...
    ArbolAVLProductos& arbolProductos, ArbolRNMarcas& arbolMarcas)
{
    ...
    while (getline(archivo, linea))
    {
        ...
        if (arbolPasillos.buscarPasillo(codPasillo) != NULL
            ...
            // Inserta en el �rbol AA de inventarios despu�s de las validaciones
...
static void crearArbolClientes(ArbolBUsuarios& arbolClientes, ArbolBBCiudades& arbolCiudades)
{
    ...
    while (getline(archivo, linea))
	{
        ...
        {
			// Inserta en el �rbol B de clientes despu�s de las validaciones
...
static void crearArbolAdmins(ArbolBUsuarios& arbolAdmins, ArbolBBCiudades& arbolCiudades)
{
    ...
    while (getline(archivo, linea))
	{
		...
		if (arbolCiudades.ciudadRepetida(codCiudad))
        {
			// Inserta en el �rbol B de administradores despu�s de las validaciones
